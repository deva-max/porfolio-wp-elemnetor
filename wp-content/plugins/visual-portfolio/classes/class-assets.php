...
class Visual_Portfolio_Assets {
    ...
    public static function enqueue_stored_assets( $type = 'script' ) {
        ...
        foreach ( self::$stored_assets[ $type ] as $name => $data ) {
            if ( isset( $data['value'] ) && $data['value'] ) {
                if ( 'script' === $type ) {
                    wp_enqueue_script( $name, '', array(), '3.1.3', true );
                    ...
                    wp_enqueue_style( $name, '', array(), '3.1.3' );
                }
    ...
    public static function enqueue( $options ) {
        ...
        if ( ! wp_style_is( $controls_css_handle, 'enqueued' ) ) {
            ...
            if ( $dynamic_styles_inline_style ) {
                wp_register_style( $controls_css_handle, false, array(), '3.1.3' );
                ...
                wp_register_script( $controls_css_handle, false, array(), '3.1.3', true );
    ...
    public function register_scripts() {
        ...
        if ( apply_filters( 'vpf_enqueue_plugin_lazysizes', true ) ) {
            wp_register_script( 'lazysizes-config', visual_portfolio()->plugin_url . 'assets/js/lazysizes-cfg.min.js', array(), '3.1.3', true );
            ...
            wp_register_script( 'lazysizes-swiper-duplicates-load', visual_portfolio()->plugin_url . 'assets/js/lazysizes-swiper-duplicates-load.min.js', array(), '3.1.3', true );
        ...
        foreach ( $vp_styles as $name => $data ) {
            wp_register_style( $name, visual_portfolio()->plugin_url . $data[0], $data[1], '3.1.3' );
        ...
        foreach ( $vp_scripts as $name => $data ) {
            wp_register_script( $name, visual_portfolio()->plugin_url . $data[0], $data[1], '3.1.3', true );
        }
